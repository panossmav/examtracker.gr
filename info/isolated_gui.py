...
class ExamTracker(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("ExamTracker - Preview Mode")
    ...
    def _show_login(self):
        ...
        tk.Label(wrap, text="Σύνδεση (Λειτουργία Προεπισκόπησης)", font=FONT_SM,
    ...
    def _do_login(self):
        ...
        # Εικονική επιτυχής σύνδεση χωρίς έλεγχο βάσης
        result = ["Καθηγητής / Διαχειριστής"] 
    ...
    def _page_stats(self):
        ...
        self._s_name.insert(0, "Γιώργος Παπαδόπουλος") # Προεπιλεγμένο για ευκολία
        ...
        self._s_subj.insert(0, "Μαθηματικά") # Προεπιλεγμένο για ευκολία
    ...
    def _stat_overall(self):
        ...
        # Mock τιμή
        avg = 16.85
        self._stat_result.config(text=f"Γενικός μ.ο. για {name}: {avg} (Εικονικά Δεδομένα)", fg=GREEN)

    def _stat_subject(self):
        name = self._s_name.get().strip()
        subj = self._s_subj.get().strip()
        if not name or not subj:
            messagebox.showerror("Απαιτείται είσοδος", "Εισάγαγε όνομα μαθητή και μάθημα.")
            return
        # Mock τιμή
        avg = 18.20
        self._stat_result.config(text=f"Μ.ο. {subj} για {name}: {avg} (Εικονικά Δεδομένα)", fg=GREEN)
    ...
    def _stat_grades(self):
        ...
        # Mock λίστα βαθμών
        grades = [15.5, 19.0, 17.5, 20.0]
        for g in grades:
            self._tree.insert("", "end", values=(name, subj, g))
        
        count = len(grades)
        self._stat_result.config(
            text=f"Εμφάνιση {count} βαθμ. για {name} / {subj} (Εικονικά Δεδομένα)",
            fg=MUTED,
        )

    # ══════════════════════════════════════════════════════════════════════════
    # ΣΕΛΙΔΑ: ΚΑΤΑΧΩΡΗΣΗ ΕΞΕΤΑΣΗΣ (MOCKED)
    # ══════════════════════════════════════════════════════════════════════════
    ...
    def _do_log_exam(self):
        ...
        if not all(vals.values()):
            messagebox.showerror("Απαιτείται είσοδος", "Όλα τα πεδία είναι υποχρεωτικά.")
            return
        try:
        ...
        # Παράκαμψη της βάσης δεδομένων
        self._le_status.config(
            text=f"✓  {vals['le_name']} · {vals['le_subj']} · {mark} καταχωρήθηκε (Εικονικά).",
            fg=GREEN,
        )
    ...
    def _page_add_student(self):
        ...
        tk.Label(f,
                 text="ℹ  Λειτουργία Προεπισκόπησης — "
                      "το αντικείμενο δημιουργείται εικονικά στην οθόνη.",
    ...
    def _do_add_student(self):
        ...
        self._as_status.config(
            text=f"✓  Student(name='{name}', age={age}, grade='{grade}') δημιουργήθηκε επιτυχώς (Εικονικά).",
            fg=GREEN,
        )
